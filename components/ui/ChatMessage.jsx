...
export const ChatMessage = memo(({ content, id, role }) => {
    const blocks = useMemo(() => parseMarkdownIntoBlocks(content), [content]);

    if (role === "user") {
        return (
            <div className="chat chat-end drop-shadow-md">
                <div className="text-md chat-bubble max-w-[70%] bg-blue-600 text-white">
                    {content &&
    ...
    return (
        <div className="chat chat-start drop-shadow-md">
            <div className={cn("text-md chat-bubble max-w-[70%] bg-gray-100 text-gray-800")}>
                {content &&
                    blocks.map((block, index) => (
                        <MemoizedMarkdownBlock content={block} key={`${id}-block_${index}`} />
                    ))}
